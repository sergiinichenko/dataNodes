...
class Socket(Serializer):
    def __init__(self, node, inout, index=0, position=LEFT_TOP, soket_type=SOCKET_TYPE_DATA, label=None):
        super().__init__()
    ...
    def remove(self):
        ...
        self.node.outputs = [output for output in self.node.outputs if output != self]
